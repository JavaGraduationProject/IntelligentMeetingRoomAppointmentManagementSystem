...
<title>共享文件</title>
<meta name="keywords" content="共享文件">
<meta name="description" content="共享文件">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<a href="${pageContext.request.contextPath}/admin/file_share/add" class="btn btn-primary">新增</a>
					</div>
					...
					<div class="table-responsive">
						<table class="table  table-striped table-bordered  table-hover">
							...
							<tbody  id="file_share_body">
							</tbody>
						</table>
						<div id="file_share_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=1720"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/file_share/queryList",
		data : {
			"page":page,
			"title":title
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var titleVal=setNullToEmpty(rows[i].fileShare.title);
				html+='<td>'+ titleVal+'</td>';
				var fintroVal=setNullToEmpty(rows[i].fileShare.fintro);
				html+='<td>'+ fintroVal+'</td>';
				var fileUrlStr = rows[i].fileShare.fileUrl;
				if(fileUrlStr!=null){
					var fileUrlSplit = fileUrlStr.split(";");
					var tmp = '';
					for(var j=0;j<fileUrlSplit.length;j++){
						if(fileUrlSplit[j]!=''){
							tmp+='<div ><a  href="javascript:void(0)" file-url="'+fileUrlSplit[j]+'" onclick="downLoadFile(this)">点击下载文件链接</a></div>';
						}
					}
					html+='<td>'+tmp+'</td>';
					}else{
				...
				var createDateVal=setNullToEmpty(rows[i].fileShare.createDate);
				html+='<td>'+ createDateVal+'</td>';
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/admin/file_share/update?id='+rows[i].fileShare.id+'" class="btn btn-success btnStyle">修改</a>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].fileShare.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/file_share/queryCount",
		async: false,
		data : {
			"title":title
		},
...
function del(e){
	if(window.confirm('你确定要删除该共享文件吗')){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/file_share/del",
			data : {
				"id":id
			},
			success : function(result) {
