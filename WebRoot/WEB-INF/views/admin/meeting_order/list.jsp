...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=7912"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/meeting_order/queryList",
		data : {
			"page":page,
			"title":title,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].meetingOrder.id+'" class="btn btn-danger btnStyle">删除</a>';
				if(rows[i].meetingOrder.meetingStatus=='1'){
					html+='<a href="javascript:void(0)" onclick="shtg(this)" data-id="'+rows[i].meetingOrder.id+'" act-type="1" class="btn btn-primary btnStyle">审核通过</a>';
				}
				if(rows[i].meetingOrder.meetingStatus=='1'){
					html+='<a href="javascript:void(0)" onclick="shbtg(this)" data-id="'+rows[i].meetingOrder.id+'" act-type="2" class="btn btn-danger btnStyle">审核不通过</a>';
				}
				if((rows[i].meetingOrder.meetingStatus=='2'||rows[i].meetingOrder.meetingStatus=='4'||rows[i].meetingOrder.meetingStatus=='5')){
					html+='<a href="javascript:void(0)" data-url="${pageContext.request.contextPath}/admin/member?meetingId='+rows[i].meetingOrder.id+'" page-name="参会人员" onclick="gotoPage(this)"  class="btn btn-warning btnStyle">参会人员</a>';
				}
				html+='<a href="javascript:void(0)" data-url="${pageContext.request.contextPath}/admin/sign_log?meetingId='+rows[i].meetingOrder.id+'" page-name="签到记录" onclick="gotoPage(this)"  class="btn btn-warning btnStyle">签到管理</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/meeting_order/queryCount",
		async: false,
		data : {
			"title":title,
			"meetingStatus":meetingStatus
...
function shtg(e){
	var r=confirm("确认审核通过该会议室预订？");
	if(r==true){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/meeting_order/shtg",
			data : {
				"id":id
			},
			success : function(result) {
...
function shbtg(e){
	var r=confirm("确认审核不通过该会议室预订？");
	if(r==true){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/meeting_order/shbtg",
			data : {
				"id":id
			},
			success : function(result) {
...
function del(e){
	if(window.confirm('你确定要删除该会议室预订吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/meeting_order/del",
			data : {
				"id":id
			},
			success : function(result) {
