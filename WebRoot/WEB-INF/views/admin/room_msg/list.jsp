...
<meta name="description" content="会议室">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="row">
						<div class="col-lg-2">
							<label>会议室编号</label>
							<input type="text" class="form-control" value="" placeholder="请输入会议室编号" id="roomNo" >
						</div>
						<div class="col-lg-2">
							<label>会议室类型</label>
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<a href="${pageContext.request.contextPath}/admin/room_msg/add" class="btn btn-primary">新增</a>
					</div>
					...
					<div class="table-responsive">
						<table class="table  table-striped table-bordered  table-hover">
							...
							<tbody  id="room_msg_body">
							</tbody>
						</table>
						<div id="room_msg_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=1039"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/room_msg/queryList",
		data : {
			"page":page,
			"roomNo":roomNo,
			"roomType":roomType
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var roomNoVal=setNullToEmpty(rows[i].roomMsg.roomNo);
				html+='<td>'+ roomNoVal+'</td>';
				var roomTypeVal=setNullToEmpty(rows[i].roomTypeStr);
				html+='<td>'+roomTypeVal+'</td>';
				var raddressVal=setNullToEmpty(rows[i].roomMsg.raddress);
				html+='<td>'+ raddressVal+'</td>';
				var rhyssbVal=setNullToEmpty(rows[i].roomMsg.rhyssb);
				html+='<td>'+ rhyssbVal+'</td>';
				var numVal=setNullToEmpty(rows[i].roomMsg.num);
				html+='<td>'+ numVal+'</td>';
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/admin/room_msg/update?id='+rows[i].roomMsg.id+'" class="btn btn-success btnStyle">修改</a>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].roomMsg.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/room_msg/queryCount",
		async: false,
		data : {
			"roomNo":roomNo,
			"roomType":roomType
...
function del(e){
	if(window.confirm('你确定要删除该会议室吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/room_msg/del",
			data : {
				"id":id
			},
			success : function(result) {
