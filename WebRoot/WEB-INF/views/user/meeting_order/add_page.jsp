...
<title>预订会议室-会议室预订</title>
<meta name="keywords" content="预订会议室-会议室预订">
<meta name="description" content="预订会议室-会议室预订">
					...
					<div class="row"   style="width:80%;margin-left:10%">
						<div class="col-lg-6">
							<div class="form-group" id="titleParentContent">
								<label>会议主题</label>
								<input type="text" value="" placeholder="请输入会议主题" id="title" class="form-control">
							</div>
							<div class="form-group" id="startTimeParentContent">
								<label>开始时间<span style="color:red">(*必填)</span></label>
								<input type="text" class="form-control Wdate"  readonly="readonly"  value=""  onFocus="WdatePicker({startDate: '%y-%M-%d 00:00:00' ,dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate: true })"  placeholder="请输入开始时间" id="startTime">
							</div>
							<div class="form-group" id="endTimeParentContent">
								<label>结束时间<span style="color:red">(*必填)</span></label>
								<input type="text" class="form-control Wdate"  readonly="readonly"  value=""  onFocus="WdatePicker({startDate: '%y-%M-%d 00:00:00' ,dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate: true })"  placeholder="请输入结束时间" id="endTime">
							</div>
							<div class="form-group" id="totalHoursParentContent">
								<label>总时长<span style="color:red">(*必填)</span></label>
								<input type="text" value="" placeholder="请输入总时长(请输入数字)" id="totalHours" class="form-control">
							</div>
							<div class="form-group" id="usingIntroParentContent">
								<label>用途<span style="color:red">(*必填)</span></label>
								<input type="text" value="" placeholder="请输入用途" id="usingIntro" class="form-control">
							</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=5912"></script>
...
function submitData(){
	...
	var usingIntro= $('#usingIntro').val();
	if(totalHours!=''){
		if(isNum(totalHours)==false){
			alert("总时长必须填入数字");
			return;
		}
	}
	$.ajax({
		type: 'post',
		url: '${pageContext.request.contextPath}/user/meeting_order/add_submit',
		data:{
			"rid":"${data.rid}",
			"title":title,
			"startTime":startTime,
		...
		success: function(result) {
			if(result.code == 0){
				...
				alert(result.msg);
				window.location.href="${pageContext.request.contextPath}/user/meeting_order?rid=${data.rid}";
			}
		}
	});
}
