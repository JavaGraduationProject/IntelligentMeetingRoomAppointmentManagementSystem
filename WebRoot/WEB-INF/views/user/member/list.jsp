...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=2528"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/member/queryList",
		data : {
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				if(rows[i].canUpdate==1){
					html+='<a href="${pageContext.request.contextPath}/user/member/update?id='+rows[i].member.id+'&meetingId=${data.meetingId}&meetingId=${data.meetingId}" class="btn btn-success btnStyle">修改</a>';
					html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].member.id+'" class="btn btn-danger btnStyle">删除</a>';	
				}
				
				if(rows[i].isSign==1){
					html+='<a href="javascript:void(0)" onclick="sign(this)" data-id="'+rows[i].member.id+'" act-type="1" class="btn btn-warning btnStyle">签到</a>';
				}
				html+='</td>';
				html+='</tr>';
			}
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/member/queryCount",
		async: false,
		data : {
			"meetingId":"${data.meetingId}",
			"title":title
...
function sign(e){
	...
	if(r==true){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/user/member/sign",
			data : {
				"id":id
			},
			success : function(result) {
...
function del(e){
	if(window.confirm('你确定要删除该参会人员吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/user/member/del",
			data : {
				"id":id
			},
			success : function(result) {
