...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=7669"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/notice_msg/queryList",
		data : {
			"page":page,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/user/notice_msg/detail2?id='+rows[i].noticeMsg.id+'" class="btn btn-primary btnStyle">查看详情</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/notice_msg/queryCount",
		async: false,
		data : {
			"noticeName":noticeName
		},
