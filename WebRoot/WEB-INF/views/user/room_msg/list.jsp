...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=7916"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/room_msg/queryList",
		data : {
			"page":page,
			"roomNo":roomNo,
			"roomType":roomType
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>'+ numVal+'</td>';
				html+='<td>';
				html+='<a href="javascript:void(0)" data-url="${pageContext.request.contextPath}/user/meeting_order/add?rid='+rows[i].roomMsg.id+'" page-name="会议室预订" onclick="gotoPage(this)"  class="btn btn-warning btnStyle">会议室预订</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/room_msg/queryCount",
		async: false,
		data : {
			"roomNo":roomNo,
			"roomType":roomType
