...
@Controller
@RequestMapping("/admin/file_share")
public class AFileShareController{
	...
	*/
	@RequestMapping("add")
	public String add(ModelMap modelMap,FileShare model,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);//从session中获取当前登录账号
		modelMap.addAttribute("data",model);
		return "admin/file_share/add_page";
	}
	/**
	新增提交信息接口
	*/
	...
	@ResponseBody
	public Object add_submit(FileShare model,ModelMap modelMap,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);
		Map<String,Object> rs = new HashMap<String,Object>();
		String msg = fileShareService.add(model,login);//执行添加操作
		if(msg.equals("")){
			rs.put("code",1);
			rs.put("msg","新增成功");
			return rs;
	...
	*/
	...
	public String update(ModelMap modelMap,FileShare model,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);//从session中获取当前登录账号
		FileShare data = fileShareMapper.selectByPrimaryKey(model.getId());
		modelMap.addAttribute("data",data);
		return "admin/file_share/update_page";
	}
	/**
	修改提交信息接口
	*/
	...
	@ResponseBody
	public Object update_submit(FileShare model,ModelMap modelMap,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);
		Map<String,Object> rs = new HashMap<String,Object>();
		String msg = fileShareService.update(model,login);//执行修改操作
		if(msg.equals("")){
			rs.put("code",1);
			rs.put("msg","修改成功");
			return rs;
